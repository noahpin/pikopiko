    ...
    function render() {
      ...
      for(let x = 0; x <= xInstances; x++) {
        for(let y = 0; y <= yInstances; y++) {
          let grayColor = chroma({h: 0, s: 0, v: noise.simplex2(x + scroll, y)/2 + .5}).hex()
          let rainbowColor = chroma({h: (noise.simplex2(x + scroll, y)/2 + .5) * 360, s: .75, v: 1})
          ctx.fillStyle = rainbowColor.hex()
          let xPos = (x*gap) - (gap / 2);
          let yPos = (y*gap) - (gap / 2);
          let distance = Math.sqrt(Math.pow(xPos - mouseX, 2) + Math.pow(yPos - mouseY, 2));
